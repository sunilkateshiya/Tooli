...
open class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    public init(sourceView: UIView, menuPosition: ENSideMenuPosition, blurStyle: UIBlurEffectStyle = .light) {
     ...
     //   sideMenuContainerView.frame.width (pass this for left side menu as x of outterview frame)
    ...
    func updateFrame() {
        ...
        outterView.frame = CGRect(x:0, y:0, width:sourceView.frame.width - sideMenuContainerView.frame.width,
                                  height:sourceView.frame.height)
    }
