...
class CompanyView: UIViewController,UITableViewDelegate,UITableViewDataSource, UIScrollViewDelegate,
NVActivityIndicatorViewable,SKPhotoBrowserDelegate {
    ...
    @IBOutlet var nodataView:UIView!
     ...
     @IBOutlet weak var JobsHeight: NSLayoutConstraint!
     ...
     override func viewDidLoad() {
          ...
          self.tblJobs.separatorStyle = .none
          self.tblJobs.rowHeight = UITableViewAutomaticDimension
          self.tblJobs.estimatedRowHeight = 450
    ...
    @IBAction func homePage(_ sender: UIButton)
    {
     ...
     @IBAction func btnMessageTapped(_ sender: UIButton)
     {
        ...
        currentBuddy.ChatUserID = userId
        currentBuddy.Name = self.ObjCompanyProfileModel.Result.CompanyName
        currentBuddy.Role = 2
        currentBuddy.IsReadLastMessage = true
        currentBuddy.ProfileImageLink = self.ObjCompanyProfileModel.Result.ProfileImageLink
     ...
     @IBAction func btnAboutTapped(_ sender: Any)
     {
         self.nodataView.isHidden = true
          ...
          self.btnJob.isSelected = false
          self.btnSpecialOffer.isSelected = false
          self.tblJobs.isHidden = true
          self.viewAboutUs.isHidden = false
          self.tblSpecialOffer.isHidden = true
          self.AboutUsHeight.constant = 2000
          self.JobsHeight.constant = 2000
          self.SpecialOfferHeight.constant = 2000
          setLayout()
     ...
     // MARK: - btnJobsTapped
     //--------------------------------------
     @IBAction func btnJobsTapped(_ sender: UIButton)
     {
          self.btnJob.isSelected = true
          self.btnSpecialOffer.isSelected = false
          self.btnAbout.isSelected = false
          self.tblJobs.isHidden = false
          self.viewAboutUs.isHidden = true
          self.tblSpecialOffer.isHidden = true
          self.JobsHeight.constant = self.tblJobs.contentSize.height
          self.AboutUsHeight.constant = self.tblJobs.contentSize.height
          self.SpecialOfferHeight.constant = self.tblJobs.contentSize.height
          setLayout()
        if(self.ObjCompanyProfileModel.Result.JobList.count == 0)
        {
     ...
     @IBAction func btnSpecialOffersTapped(_ sender: UIButton)
     {
          ...
          self.JobsHeight.constant = self.tblSpecialOffer.contentSize.height
    ...
    {
        let vc  = self.storyboard?.instantiateViewController(withIdentifier: "JobListVC") as! JobListVC
        vc.userid = self.userId
        vc.username = self.ObjCompanyProfileModel.Result.CompanyName
     ...
     func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
     {
          ...
          else if tableView == tblJobs
          {
               return self.ObjCompanyProfileModel.Result.JobList.count
          }
          else
          {
     ...
     func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
     {
          ...
          else if tableView == tblJobs
          {
            ...
            cell.lblJobRole.text = self.ObjCompanyProfileModel.Result.JobList[indexPath.row].JobRoleName
            cell.lblDis.text = self.ObjCompanyProfileModel.Result.JobList[indexPath.row].CompanyName as String!
            cell.lblTradename.text = self.ObjCompanyProfileModel.Result.JobList[indexPath.row].JobTradeName
            
            cell.btnFav.tag=indexPath.row
            cell.btnFav.addTarget(self, action: #selector(btnfavJob(_:)), for: UIControlEvents.touchUpInside)
            if self.ObjCompanyProfileModel.Result.JobList[indexPath.row].IsSaved == true {
                cell.btnFav.isSelected = true
            }
            else{
                cell.btnFav.isSelected = false
            }
            if self.ObjCompanyProfileModel.Result.JobList[indexPath.row].ProfileImageLink != ""
            {
                let imgURL = self.ObjCompanyProfileModel.Result.JobList[indexPath.row].ProfileImageLink as String!
          ...
          else
          {
            ...
            cell.likeHeight.constant = 0
               ...
               cell.lblTitle.text = self.ObjCompanyProfileModel.Result.OfferList[indexPath.row].CompanyName
               cell.lblDis.text = self.ObjCompanyProfileModel.Result.OfferList[indexPath.row].Description
               cell.lblDate.text = self.ObjCompanyProfileModel.Result.OfferList[indexPath.row].Title + "-" + self.ObjCompanyProfileModel.Result.OfferList[indexPath.row].PriceTag
            if(self.ObjCompanyProfileModel.Result.OfferList[indexPath.row].IsSaved)
            {
          ...
          AFWrapper.requestPOSTURL(Constants.URLS.CompanyProfileView, params :param as [String : AnyObject]? ,headers : nil  ,  success:
               {
                    ...
                    if JSONResponse["Status"].int == 1
                    {
                        ...
                        else if(self.btnJob.isSelected)
                         {
                             self.btnJobsTapped(self.btnJob)
                        }
                        else
                         {
                            self.btnSpecialOffersTapped(self.btnSpecialOffer)
                        }
                    }
                    else
                    {
          ...
          DispatchQueue.main.async
               {
                    if self.btnAbout.isSelected
                    {
                         self.ObjScrollView.contentSize.height = self.tblJobs.frame.origin.y + self.tblSkill.frame.origin.y + self.tblSkill.frame.height + 20
                    }
                    else if self.btnJob.isSelected
                    {
                         self.ObjScrollView.contentSize.height =
                              self.tblJobs.frame.origin.y +  self.tblJobs.contentSize.height
                    }
                    else
                    {
                         self.ObjScrollView.contentSize.height =
                              self.tblJobs.frame.origin.y +  self.tblSpecialOffer.contentSize.height
                    }
          }
     }
    ...
    func btnfavJob(_ sender : UIButton)
    {
        self.startAnimating()
        let param = ["TablePrimaryID":"\(self.ObjCompanyProfileModel.Result.JobList[sender.tag].JobID)",
        ...
        AFWrapper.requestPOSTURL(Constants.URLS.SaveToggle, params :param as [String : AnyObject]? ,headers : nil  ,  success: {
            ...
            if JSONResponse["Status"].int == 1
            {
                ...
                {
                    self.ObjCompanyProfileModel.Result.JobList[sender.tag].IsSaved = false
                }
                else
                {
                    self.ObjCompanyProfileModel.Result.JobList[sender.tag].IsSaved = true
                }
                self.tblJobs.reloadData()
            }
            else
